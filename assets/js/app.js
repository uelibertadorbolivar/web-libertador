/**
 * SIGAE v3.1 - JAVASCRIPT SPA
 * REVISIÓN FINAL: Cambio de Contraseña y Cierre de Loaders
 */
...
const App = {
    ...
    login: function() {
        ...
        App.sendRequest({ action: "login", cedula: ced, password: pass }, (res) => {
            ...
            else { this.hideLoader(); Swal.fire('Error', 'Credenciales incorrectas', 'error'); }
        });
    },
    ...
    verificarUsuario: function() {
        ...
        App.sendRequest({ action: "verificar_usuario", cedula: ced }, (res) => {
            ...
            } else { Swal.fire('No Autorizado', 'Cédula no registrada.', 'error'); }
        });
    },
                ...
                <div class="col-12">
                    <div class="card p-5 text-white shadow-lg border-0 position-relative overflow-hidden" style="background: linear-gradient(135deg, #00C3FF 0%, #0085FF 100%); border-radius: 30px;">
                        <div class="position-relative z-1">
                            ...
                            <button onclick="App.descargarOrganigramaPDF()" class="btn btn-light text-primary fw-bold rounded-pill px-4 shadow">
                                <i class="bi bi-file-earmark-pdf-fill me-2"></i> Descargar Organigrama (PDF)
                            </button>
                        </div>
                    </div>
                </div>
                <div class="col-md-3"><div class="dash-card border-left-blue"><h6>Misión</h6><p class="small text-muted mb-0 text-truncate-3">${res.mision || '--'}</p></div></div>
                <div class="col-md-3"><div class="dash-card border-left-green"><h6>Visión</h6><p class="small text-muted mb-0 text-truncate-3">${res.vision || '--'}</p></div></div>
                <div class="col-md-3"><div class="dash-card border-left-yellow"><h6>Objetivo</h6><p class="small text-muted mb-0 text-truncate-3">${res.objetivo || '--'}</p></div></div>
                <div class="col-md-3"><div class="dash-card border-left-red"><h6>PEIC</h6><p class="small text-muted mb-0 text-truncate-3">${res.peic || '--'}</p></div></div>
            </div>
        </div>`;
    },
    ...
    sendRequest: function(data, callback) { 
        ...
        .catch(e => { console.error(e); this.hideLoader(); Swal.fire('Error', 'Fallo de conexión o servidor', 'error'); }); 
    },
        ...
        App.sendRequest({ action: "get_supervision" }, (supRes) => {
            ...
            setTimeout(() => {
                html2canvas(tempDiv, { scale: 2, useCORS: true, width: tempDiv.scrollWidth, height: tempDiv.scrollHeight }).then(canvas => {
                    ...
                    pdf.save(`Organigrama_SIGAE.pdf`);
                    document.body.removeChild(tempDiv);
                    App.hideLoader();
                    Swal.fire('Éxito', 'Archivo generado', 'success');
                });
            }, 1500);
        });
    },
...
const ModuloEscuela = {
    ...
    save: function() { 
        ...
        App.showLoader(); 
        App.sendRequest({ action: "save_school_profile", data }, (res) => { 
            App.hideLoader(); 
            if(res.status==='success') { 
                Swal.fire('Éxito','Datos del plantel actualizados','success');
                App.sendRequest({ action: "get_school_profile" }, (resS) => { App.schoolData = resS; this.render(document.getElementById('module-content')); }); 
            }
        }); 
    }
};
...
const ModuloConfiguracion = {
    ...
    render: function(cont) {
        cont.innerHTML = `<div class="row g-4 animate__animated animate__fadeIn w-100 m-0"><div class="col-lg-7"><div class="card border-0 shadow-sm p-4 rounded-4 bg-white h-100"><div class="d-flex justify-content-between mb-4"><h6>Años Escolares</h6><button onclick="ModuloConfiguracion.modalAnio()" class="btn btn-sm btn-primary-vibrant rounded-pill px-3 shadow-sm">+ Nuevo</button></div><div class="table-responsive"><table class="table table-hover align-middle"><thead><tr class="table-light"><th>Nombre</th><th>Estado</th><th class="text-end">Acción</th></tr></thead><tbody id="tbl-anios"></tbody></table></div></div></div><div class="col-lg-5"><div class="card border-0 shadow-sm p-4 rounded-4 bg-white h-100"><div class="d-flex justify-content-between mb-4"><h6>Lapsos / Fases</h6><button onclick="ModuloConfiguracion.modalPeriodo()" class="btn btn-sm btn-warning-vibrant rounded-pill px-3 shadow-sm">+ Agregar</button></div><div id="list-periodos" class="d-flex flex-column gap-2"></div></div></div></div>`;
        this.load();
    },
    ...
    draw: function() {
        const t = document.getElementById('tbl-anios'); if(t) t.innerHTML = this.anios.map(a => `<tr><td class="fw-bold">${a.nombre}</td><td><div class="form-check form-switch"><input class="form-check-input" type="checkbox" onchange="ModuloConfiguracion.setActual('${a.id}')" ${a.estado === 'Actual' ? 'checked' : ''}> <small>${a.estado}</small></div></td><td class="text-end"><button onclick="ModuloConfiguracion.modalAnio('${a.id}')" class="btn btn-sm btn-light text-primary rounded-circle"><i class="bi bi-pencil"></i></button><button onclick="ModuloConfiguracion.delete('${a.id}','ANIO')" class="btn btn-sm btn-light text-danger rounded-circle ms-1"><i class="bi bi-trash"></i></button></td></tr>`).join('');
        const l = document.getElementById('list-periodos'); if(l) l.innerHTML = this.periodos.map(p => `<div class="d-flex justify-content-between align-items-center p-3 bg-light rounded-4 border-start border-4 border-warning shadow-sm"><span>${p.nombre}</span><div><button onclick="ModuloConfiguracion.modalPeriodo('${p.id}','${p.nombre}')" class="btn btn-sm btn-link p-0 me-2"><i class="bi bi-pencil"></i></button><button onclick="ModuloConfiguracion.delete('${p.id}','PERIODO')" class="btn btn-sm btn-link text-danger p-0"><i class="bi bi-trash"></i></button></div></div>`).join('');
    },
    modalAnio: function(id=null){ 
        const d = id?this.anios.find(x=>x.id===id):{nombre:''}; 
        Swal.fire({ title:'Año Escolar', input:'text', inputValue: d.nombre, showCancelButton:true }).then(r=>{ if(r.isConfirmed){ App.showLoader(); App.sendRequest({ action:"save_school_year", year:{id, nombre:r.value}}, (res)=>{ App.hideLoader(); if(res.status==='success') { Swal.fire('Éxito','Año guardado','success'); this.load(); }}); }});
    },
    modalPeriodo: function(id=null, n=''){ Swal.fire({ title:'Lapso', input:'text', inputValue:n, showCancelButton:true }).then(r=>{ if(r.isConfirmed){ App.showLoader(); App.sendRequest({ action:"save_school_period", period:{id, nombre:r.value}}, (res)=>{ App.hideLoader(); if(res.status==='success') { Swal.fire('Éxito','Lapso guardado','success'); this.load(); }}); }}); },
...
const ModuloNiveles = {
    niveles:[],
    render: function(cont) { cont.innerHTML = `<div class="col-12 card border-0 shadow-sm p-4 rounded-4 bg-white w-100"><div class="d-flex justify-content-between mb-4 flex-wrap gap-2"><h6>Niveles Educativos</h6><button onclick="ModuloNiveles.modal()" class="btn btn-sm btn-primary-vibrant rounded-pill px-4 shadow-sm">+ Agregar</button></div><div class="table-responsive"><table class="table align-middle w-100"><thead><tr class="table-light"><th>Nombre</th><th class="text-end px-4">Acción</th></tr></thead><tbody id="tbl-niveles"></tbody></table></div></div>`; this.load(); },
...
const ModuloGrados = {
    data:[],
    render: function(cont) { cont.innerHTML = `<div class="col-12 card border-0 shadow-sm p-4 rounded-4 bg-white mb-4 flex-wrap gap-2 d-flex flex-row justify-content-between align-items-center"><h6>Grados / Años</h6><button onclick="ModuloGrados.modal()" class="btn btn-primary-vibrant rounded-pill px-4 shadow-sm">+ Nuevo</button></div><div id="list-grados" class="row g-4 w-100 m-0"></div>`; this.load(); },
...
const ModuloDocentes = {
    ...
    renderAsignarCargo: function(cont) {
        cont.innerHTML = `<div class="col-12 card border-0 shadow-sm p-4 rounded-4 bg-white mb-4 animate__animated animate__fadeIn"><h6 class="fw-bold text-primary mb-4"><i class="bi bi-person-badge-fill me-2"></i>Asignación de Cargo</h6><div class="row g-3 align-items-end"><div class="col-md-5"><label class="small fw-bold text-muted ps-2">Personal Sin Cargo</label><select id="staff-select" class="form-select rounded-pill shadow-none"></select></div><div class="col-md-5"><label class="small fw-bold text-muted ps-2">Cargo</label><select id="cargo-select" class="form-select rounded-pill shadow-none"></select></div><div class="col-md-2"><button onclick="ModuloDocentes.saveAssignment()" class="btn btn-primary-vibrant w-100 rounded-pill fw-bold shadow-sm">Asignar</button></div></div></div><div class="col-12 card border-0 shadow-sm p-4 rounded-4 bg-white"><div class="table-responsive"><table class="table table-hover align-middle w-100"><thead class="table-light"><tr><th>Personal</th><th>Cédula</th><th>Cargo</th><th class="text-end px-4">Acción</th></tr></thead><tbody id="tbl-assignments"></tbody></table></div></div>`;
    ...
    drawAssignments: function() { const tbody = document.getElementById('tbl-assignments'); if(!tbody) return; tbody.innerHTML = this.assignments.length === 0 ? '<tr><td colspan="4" class="text-center text-muted py-4">Sin asignaciones</td></tr>' : this.assignments.map(a => `<tr><td class="fw-bold">${a.nombre}</td><td>V-${a.cedula}</td><td><span class="badge bg-primary-subtle text-primary border px-3">${a.cargoNombre}</span></td><td class="text-end px-4"><button onclick="ModuloDocentes.deleteAssignment('${a.cedula}')" class="btn btn-sm btn-light text-danger rounded-circle shadow-sm"><i class="bi bi-trash-fill"></i></button></td></tr>`).join(''); },
...
const ModuloUsuarios = {
    data:[],
    render: function(cont) { cont.innerHTML = `<div class="col-12 card border-0 shadow-sm p-4 rounded-4 bg-white mb-4 w-100 animate__animated animate__fadeIn"><div class="d-flex justify-content-between align-items-center mb-3"><h6 class="fw-bold mb-0 text-primary">Gestión de Usuarios</h6><div class="d-flex gap-2"><input type="file" id="import-excel" accept=".xlsx, .xls" style="display:none" onchange="ModuloUsuarios.procesarExcel(this)"><button onclick="document.getElementById('import-excel').click()" class="btn btn-outline-success rounded-pill px-3 shadow-sm btn-sm">Importar Excel</button></div></div><div class="row g-3 mt-1"><div class="col-md-3"><input id="u-c" type="number" class="form-control rounded-pill" placeholder="Cédula"></div><div class="col-md-4"><input id="u-n" type="text" class="form-control rounded-pill" placeholder="Nombre"></div><div class="col-md-3"><select id="u-r" class="form-select rounded-pill">${Object.keys(App.allRoles).map(r=>`<option value="${r}">${r}</option>`).join('')}<option value="Administrador">Administrador</option></select></div><div class="col-md-2"><button onclick="ModuloUsuarios.save()" class="btn btn-primary-vibrant w-100 rounded-pill shadow-sm">Autorizar</button></div></div></div><div class="col-12 card border-0 shadow-sm p-4 rounded-4 bg-white w-100"><div class="table-responsive"><table class="table table-hover align-middle w-100"><thead><tr class="table-light"><th>Cédula</th><th>Nombre</th><th>Rol</th><th class="text-end px-4">Acción</th></tr></thead><tbody id="tbl-u"></tbody></table></div></div>`; this.load(); },
    ...
    save: function(){ 
        ...
        App.showLoader(); App.sendRequest({action:"save_user", user:u}, (res)=>{ App.hideLoader(); if(res.status==='success') { Swal.fire('Éxito','Usuario autorizado','success'); this.load(); } else { Swal.fire('Error',res.message,'error'); }}); 
    },
    procesarExcel: function(input) {
...
const ModuloSoftware = {
    ...
    formRole: function(n="", p=[]) {
        ...
        Swal.fire({ title:'Permisos', width:'850px', html: `<input id="r-n" class="form-control rounded-pill mb-3" placeholder="Nombre Rol" value="${n}" ${n?'disabled':''}><div class="row text-start" style="max-height:400px; overflow-y:auto; border: 1px solid #eee; padding: 20px; border-radius: 15px;">${o}</div>`, showCancelButton:true, confirmButtonText:'Guardar' }).then(res => { if(res.isConfirmed){ 
            const nV = document.getElementById('r-n').value, pV = Array.from(document.querySelectorAll('.r-chk:checked')).map(c=>c.value);
            App.showLoader(); App.sendRequest({ action:"save_role", nombre:nV, permisos:pV}, (res)=>{ App.hideLoader(); if(res.status==='success') { App.loadAppData(); }}); 
        }});
    },
    ...
    deleteRole: function(r){ Swal.fire({ title:'¿Eliminar?', showCancelButton:true }).then(res => { if(res.isConfirmed){ App.showLoader(); App.sendRequest({ action:"delete_role", nombre:r }, (res)=>{ App.hideLoader(); if(res.status==='success') { App.loadAppData(); }}); }}); }
};
...
const ModuloPerfil = {
    ...
    save: function() { 
        ...
        App.sendRequest({ action: "update_user_profile", user: u }, (res) => { 
            App.hideLoader(); 
            if(res.status==='success') { 
                App.user.name = u.nombre; 
                localStorage.setItem('schoolUser', JSON.stringify(App.user)); 
