...
const App = {
    ...
    loadAppData: function() {
        ...
        App.sendRequest({ action: "get_roles" }, (resRoles) => {
            ...
            // --- GESTIÓN DE PRIVILEGIOS DE VISITANTE ---
            if(this.user && this.user.role === "Visitante") {
                if (!this.allRoles["Visitante"]) {
                    // Si no está configurado en la base de datos, usamos estos por defecto
    ...
    sendRequest: function(data, callback) { 
        ...
        .catch(e => { console.error(e); this.hideLoader(); Swal.fire('Error', 'Sin conexión con el servidor.', 'error'); }); 
    },
...
const ModuloOrganigrama = {
    render: function(cont) {
        cont.innerHTML = `<div class="row animate__animated animate__fadeIn w-100 m-0"><div class="col-12 card border-0 shadow-sm p-4 rounded-4 bg-white mb-4 d-flex flex-row justify-content-between align-items-center flex-wrap gap-2"><h6>Organigrama Institucional</h6><button onclick="ModuloOrganigrama.download()" class="btn btn-success-vibrant rounded-pill px-4 fw-bold shadow-sm">Descargar PNG</button></div><div class="col-12 bg-white rounded-4 shadow-sm p-5 overflow-auto text-center" id="capture-zone" style="min-height:600px;"><div class="org-tree-wrapper" id="org-tree-container"></div></div></div>`;
        this.load();
    },
