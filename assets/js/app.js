const systemStructure = [
    { category: "Escuela", icon: "bi-building", color: "#FF8D00", items: ["Perfil de la Escuela", "Fases y Periodos", "Niveles Educativos", "Escalas de Evaluación", "Gestión de Cargos", "Cadena Supervisoria", "Organigrama Institucional", "Grado / Año"] },
...
const App = {
    ...
    loadAppData: function() {
        ...
        App.sendRequest({ action: "get_roles" }, (resRoles) => {
            ...
            // Inyectar permisos de invitado si aplica
    ...
    verificarUsuario: function() {
        ...
        App.sendRequest({ action: "verificar_usuario", cedula: ced }, (res) => {
            ...
            } else { Swal.fire('Error', 'No autorizado', 'error'); }
        });
    },
    ...
    showDashboard: function() {
        ...
        dash.innerHTML = `<div class="p-4 animate__animated animate__fadeIn"><div class="row g-4"><div class="col-12"><div class="card p-5 text-white shadow-lg border-0 position-relative overflow-hidden" style="background: linear-gradient(135deg, #00C3FF 0%, #0085FF 100%); border-radius: 30px;"><div class="position-relative z-1"><span class="badge bg-white text-primary mb-3 px-3 py-2 rounded-pill fw-bold">Año Escolar ${this.activeYear}</span><h2 class="fw-bold mb-1">${res.nombre}</h2><p class="opacity-75 mb-0"><i class="bi bi-geo-alt-fill me-1"></i> ${res.direccion || '---'}</p></div></div></div><div class="col-md-3"><div class="dash-card border-left-blue"><h6>Misión</h6><p class="small text-muted mb-0">${res.mision || '--'}</p></div></div><div class="col-md-3"><div class="dash-card border-left-green"><h6>Visión</h6><p class="small text-muted mb-0">${res.vision || '--'}</p></div></div><div class="col-md-3"><div class="dash-card border-left-yellow"><h6>Objetivo</h6><p class="small text-muted mb-0">${res.objetivo || '--'}</p></div></div><div class="col-md-3"><div class="dash-card border-left-red"><h6>PEIC</h6><p class="small text-muted mb-0">${res.peic || '--'}</p></div></div></div></div>`;
    },
    ...
    renderView: function(id, title) {
        ...
        else if(id === 'Organigrama Institucional') { ModuloOrganigrama.render(cont); }
    ...
    sendRequest: function(data, callback) { 
        ...
        .catch(e => { console.error(e); this.hideLoader(); Swal.fire('Error', 'Fallo de red', 'error'); }); 
    },
...
const ModuloConfiguracion = {
    ...
    modalAnio: function(id=null){ 
        ...
        Swal.fire({ title:'Año Escolar', html:`<input id="sw-a" class="form-control rounded-pill mb-2" value="${d.nombre||''}">`, showCancelButton:true }).then(r=>{ if(r.isConfirmed){ App.showLoader(); App.sendRequest({ action:"save_school_year", year:{id, nombre:document.getElementById('sw-a').value}}, (res)=>{ if(res.status==='success') { Swal.fire('Éxito','Año guardado','success'); ModuloConfiguracion.load(); }}); }});
    },
    modalPeriodo: function(id=null, n=''){ Swal.fire({ title:'Lapso', input:'text', inputValue:n, showCancelButton:true }).then(r=>{ if(r.isConfirmed){ App.showLoader(); App.sendRequest({ action:"save_school_period", period:{id, nombre:r.value}}, (res)=>{ if(res.status==='success') { Swal.fire('Éxito','Lapso guardado','success'); ModuloConfiguracion.load(); }}); }}); },
    setActual: function(id){ App.showLoader(); App.sendRequest({ action:"set_current_year", id }, (res)=>{ if(res.status==='success') { Swal.fire('Actualizado','Ciclo escolar cambiado','success'); App.loadAppData(); ModuloConfiguracion.load(); }}); },
...
const ModuloCargos = {
    data:[],
    render: function(cont) { cont.innerHTML = `<div class="col-12 card border-0 shadow-sm p-4 rounded-4 bg-white w-100"><div class="d-flex justify-content-between mb-4 flex-wrap gap-2"><h6>Cargos</h6><button onclick="ModuloCargos.modal()" class="btn btn-sm btn-primary-vibrant rounded-pill px-4 shadow-sm">+ Nuevo</button></div><div class="table-responsive"><table class="table table-hover align-middle w-100"><thead><tr class="table-light"><th>Cargo</th><th>Tipo</th><th class="text-end px-4">Acción</th></tr></thead><tbody id="tbl-cargos"></tbody></table></div></div>`; this.load(); },
...
const ModuloSupervision = {
    ...
    modal: function(supId=null){
        ...
        Swal.fire({ title:'Jerarquía', html, showCancelButton:true, width:'600px' }).then(r=>{ if(r.isConfirmed){
            ...
            App.showLoader(); App.sendRequest({ action:"save_supervision", supervisorId:sId, supervisadosIds:ch }, (res)=>{ if(res.status==='success') { Swal.fire('Listo','Jerarquía guardada','success'); ModuloSupervision.load(); }});
        }});
    },
    delete: function(id){ Swal.fire({ title:'¿Eliminar?', icon:'warning', showCancelButton:true }).then(r=>{ if(r.isConfirmed){ App.showLoader(); App.sendRequest({ action:"delete_supervision", supervisorId:id }, (res)=>{ if(res.status==='success') { Swal.fire('Eliminado','','success'); ModuloSupervision.load(); }}); }}); }
};

const ModuloOrganigrama = {
    render: function(cont) {
        cont.innerHTML = `<div class="row animate__animated animate__fadeIn w-100 m-0"><div class="col-12 card border-0 shadow-sm p-4 rounded-4 bg-white mb-4 d-flex flex-row justify-content-between align-items-center"><h6>Organigrama</h6><button onclick="ModuloOrganigrama.download()" class="btn btn-success-vibrant rounded-pill px-4 fw-bold">Descargar PNG</button></div><div class="col-12 bg-white rounded-4 shadow-sm p-5 overflow-auto text-center" id="capture-zone" style="min-height:600px;"><div class="org-tree-wrapper" id="org-tree-container"></div></div></div>`;
        this.load();
    },
    load: function() { App.showLoader(); App.sendRequest({ action: "get_supervision" }, (res) => { this.buildTree(res.data || []); App.hideLoader(); }); },
    buildTree: function(data) {
        const container = document.getElementById('org-tree-container'); container.innerHTML = "";
        const supervisadosIds = new Set();
        data.forEach(s => s.supervisadosIds.forEach(id => supervisadosIds.add(String(id))));
        const roots = App.allCargos.filter(c => !supervisadosIds.has(String(c.id)));
        if (roots.length === 0 && App.allCargos.length > 0) this.drawNode(App.allCargos[0], data, container);
        else roots.forEach(root => this.drawNode(root, data, container));
    },
    drawNode: function(cargo, data, parent) {
        const nodeGroup = document.createElement('div'); nodeGroup.className = 'org-node-group';
        nodeGroup.innerHTML = `<div class="org-card shadow-sm border rounded-4 p-3 bg-white mx-2 mb-4 d-inline-block" style="min-width:200px;"><div class="small fw-bold text-primary mb-1">${cargo.tipo}</div><div class="fw-bold text-dark">${cargo.nombre}</div></div>`;
        ...
        if (rel && rel.supervisadosIds.length > 0) {
            const childrenWrapper = document.createElement('div'); childrenWrapper.className = 'org-children d-flex justify-content-center';
            nodeGroup.appendChild(childrenWrapper);
            rel.supervisadosIds.forEach(id => {
                const child = App.allCargos.find(c => String(c.id) === String(id));
                if (child) this.drawNode(child, data, childrenWrapper);
            });
        }
    },
    download: function() {
        const zone = document.getElementById('capture-zone');
        html2canvas(zone, { backgroundColor: "#ffffff", scale: 2 }).then(canvas => {
            const a = document.createElement('a'); a.download = 'organigrama.png'; a.href = canvas.toDataURL(); a.click();
        });
    }
};
...
const ModuloSoftware = {
    renderRoles: function(cont) { 
        ...
        cont.innerHTML = `<div class="d-flex justify-content-between mb-4 flex-wrap gap-2 w-100"><h5>Roles y Privilegios</h5><button onclick="ModuloSoftware.formRole()" class="btn btn-primary px-4 rounded-pill">Nuevo Rol</button></div><div class="row w-100 m-0">${h}</div>`; 
    },
    formRole: function(n="", p=[]) {
        ...
        Swal.fire({ title:'Permisos', width:'850px', html: `<input id="r-n" class="form-control rounded-pill mb-3" value="${n}" ${n?'disabled':''}><div class="row text-start" style="max-height:400px; overflow-y:auto;">${o}</div>`, showCancelButton:true }).then(res => { if(res.isConfirmed){ 
...
const ModuloPerfil = {
    render: function(cont) { App.showLoader(); App.sendRequest({ action: "get_user_full_data", cedula: App.user.cedula }, (res) => { App.hideLoader(); if (res.status === "success") { const u = res.user; cont.innerHTML = `<div class="col-12 card border-0 shadow-sm rounded-4 bg-white overflow-hidden w-100"><div class="p-4 bg-light border-bottom d-flex align-items-center"><div style="width:60px; height:60px; background:var(--brand-blue); color:white; border-radius:50%;" class="d-flex align-items-center justify-content-center me-3 fs-3 fw-bold">${u.nombre.charAt(0)}</div><div><h5 class="fw-bold mb-0">${u.nombre}</h5><span class="badge bg-primary px-3">${u.rol}</span></div></div><div class="card-body p-4"><div class="row g-3"><div class="col-md-6"><label class="small fw-bold">Cédula</label><input class="form-control rounded-pill bg-light" value="${u.cedula}" disabled></div><div class="col-md-6"><label class="small fw-bold">Nombre</label><input id="p-n" class="form-control rounded-pill" value="${u.nombre}"></div><div class="col-md-6"><label class="small fw-bold">Nueva Clave</label><input id="p-p" type="password" class="form-control rounded-pill"></div><div class="col-md-6"><label class="small fw-bold">Respuesta</label><input id="p-r" class="form-control rounded-pill" value="${u.respuesta || ''}"></div><div class="col-12 text-end"><button onclick="ModuloPerfil.save()" class="btn btn-primary px-5 rounded-pill shadow">Actualizar Mis Datos</button></div></div></div></div>`; }}); },
    save: function() { const u = { cedula: App.user.cedula, nombre: document.getElementById('p-n').value, password: document.getElementById('p-p').value, respuesta: document.getElementById('p-r').value }; App.showLoader(); App.sendRequest({ action: "update_user_profile", user: u }, (res) => { if(res.status==='success') { App.user.name = u.nombre; localStorage.setItem('schoolUser', JSON.stringify(App.user)); Swal.fire('Éxito','Perfil actualizado','success'); ModuloPerfil.render(document.getElementById('module-content')); App.hideLoader(); }}); }
};
