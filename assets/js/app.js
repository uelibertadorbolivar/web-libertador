 ...
 * VERSIÓN ROBUSTA: PWA + PERMANENCIA TRAS GUARDADO
 */

const systemStructure = [
    { category: "Escuela", icon: "bi-building", color: "#FFB81D", items: ["Perfil de la Escuela", "Fases y Periodos", "Niveles Educativos", "Escalas de Evaluación", "Gestión de Cargos", "Organigrama Institucional", "Proyecto PEIC", "Grado / Año", "Secciones"] },
    { category: "Docentes", icon: "bi-person-badge-fill", color: "#56B458", items: ["Agregar Docentes", "Asignar Cargos", "Registrar Tipos de Ausencias (Doc)", "Registrar Ausencias (Doc)", "Expediente Docente", "Condición Salud (Doc)", "Reporte Gestión Diaria (Doc)", "Planes de Evaluación"] },
    { category: "Estudiantes", icon: "bi-people-fill", color: "#5D86E6", items: ["Agregar Estudiante (Nuevo)", "Agregar Estudiante (Regular)", "Asignar Cargos (Est)", "Registrar Tipos de Ausencias (Est)", "Registrar Ausencias (Est)", "Expediente Estudiante", "Condición Salud (Est)", "Voceros Estudiantiles"] },
    { category: "Representantes", icon: "bi-person-heart", color: "#F24444", items: ["Solicitudes (Retiro/Notas/Constancia)", "Actualización de Datos (Regular)", "Inscripciones (Nuevos)", "Ver Boleta", "Ver Corte de Calificaciones"] },
    { category: "Administrativo", icon: "bi-clipboard-check-fill", color: "#8D24AA", items: ["Accidentes/Casiaccidentes (Doc)", "Accidentes/Casiaccidentes (Est)", "Reporte Preliminar Asistencias", "Reporte Incidentes y Requerimientos", "Gestión Diaria (Revisada)", "Gestión Mensual (DEP/CDV)", "Requerimientos Anuales", "Registros de Espacios", "Asignación de Espacios", "Agregar Colectivos Docentes", "Asignación de Colectivos", "Crear Salones", "Estadísticas"] },
    { category: "Transporte Escolar", icon: "bi-bus-front-fill", color: "#FFB81D", items: ["Agregar Rutas", "Agregar Paradas", "Asignación de Recorridos", "Registro de Asistencias (Transp)", "Reporte Incidentes (Est/Transp)", "Reporte Incidentes (Servicio)"] },
    { category: "Pedagogía", icon: "bi-journal-bookmark-fill", color: "#56B458", items: ["Agregar PA", "Perfil del Aula", "Calendario Escolar"] },
    { category: "Control de Estudios", icon: "bi-shield-lock-fill", color: "#8D24AA", items: ["Solicitud de Cupos", "Carnet Estudiantil", "Calificaciones", "Matrículas", "Boletas", "Retiros", "Notas Certificadas", "Certificación Títulos", "Constancias de Estudio", "Cortes de Notas", "Sábanas", "Resúmenes Finales"] },
    { category: "C.R.A.", icon: "bi-book-fill", color: "#5D86E6", items: ["Ubicaciones CRA", "Recursos Educativos CRA", "Préstamos"] },
    { category: "Formaciones", icon: "bi-patch-check-fill", color: "#56B458", items: ["Planificación de Formación", "Recursos Educativos Formación", "Descarga de Certificados"] },
    { category: "Software", icon: "bi-gear-wide-connected", color: "#F24444", items: ["Gestión de Usuarios", "Roles y Privilegios", "Cambio de Contraseñas"] },
    { category: "Diseño Web", icon: "bi-palette-fill", color: "#1e2b5e", items: ["Accesibilidad y Apariencia"] }
];
...
const App = {
    ...
    checkSession: function() {
        ...
        if (stored) {
            ...
            // AL REFRESCAR: Recuperar la última vista si existía
            this.currentView = localStorage.getItem('lastView') || 'Inicio';
            this.loadAppData();
        } else {
    ...
    showApp: function() {
        ...
        // CORRECCIÓN: Tras cargar, volver a la vista donde estaba el usuario
        if(this.currentView === 'Inicio') this.showDashboard();
        else App.renderView(this.currentView, this.currentView);
    },
    ...
    renderMenu: function() {
        ...
        let html = `<div class="px-3 mb-2"><button onclick="App.renderView('Inicio','Panel de Control')" class="nav-category-btn rounded-pill border-0 w-100" style="background: #ebf4ff; color: #1e2b5e;"><span><i class="bi bi-house-door-fill me-2"></i> Inicio</span></button></div>`;
    ...
    showDashboard: function() {
        ...
        localStorage.setItem('lastView', 'Inicio');
        ...
        const res = this.schoolData || { nombre: "UE Libertador Bolívar", direccion: "Cargando..." };
        dash.innerHTML = `
    ...
    renderView: function(id, title) {
        ...
        // PERSISTENCIA: Guardar la vista actual
        this.currentView = id;
    ...
    sendRequest: function(data, callback) {
        ...
        .then(r => r.json()).then(res => callback(res)).catch(e => { this.hideLoader(); });
    },
...
const ModuloUsuarios = {
    ...
    save: function() {
        ...
        App.sendRequest({ action: "save_user", user: user }, () => {
            ...
            this.render(); // CORRECCIÓN: Recarga la misma vista
        });
    },
    delete: function(ced) {
        Swal.fire({ title: '¿Eliminar?', icon: 'warning', showCancelButton: true }).then(r => {
            if(r.isConfirmed) { App.showLoader(); App.sendRequest({ action: "delete_user", cedula: ced }, () => this.render()); }
        });
    }
};
...
const ModuloSoftware = {
            ...
            if (result.isConfirmed) { 
                ...
                App.sendRequest({ action: "save_role", nombre: result.value.nombre, permisos: result.value.permisos }, () => { 
                    App.sendRequest({ action: "get_roles" }, (resRoles) => {
                        ...
                        this.renderRoles(); // CORRECCIÓN: Recarga la misma vista
                        App.hideLoader();
    ...
    deleteRole: function(r) { Swal.fire({ title: '¿Eliminar Rol?', icon: 'warning', showCancelButton: true }).then((res) => { if (res.isConfirmed) { App.showLoader(); App.sendRequest({ action: "delete_role", nombre: r }, () => { App.sendRequest({ action: "get_roles" }, (resRoles) => { App.allRoles = resRoles.roles || {}; this.renderRoles(); App.hideLoader(); }); }); } }); }
};
...
const Simon = {
    ...
    send: function() { const i = document.getElementById('simon-input'); if(!i.value) return; const b = document.getElementById('simon-messages'); const d = document.createElement('div'); d.className = "simon-msg user"; d.innerText = i.value; b.appendChild(d); b.scrollTop = b.scrollHeight; i.value = ""; },
    handleKey: function(e) { if(e.key === 'Enter') this.send(); }
};

const Acc = {
    currentSize: 16,
    init: function() { if(localStorage.getItem('acc-dark') === 'true') document.body.classList.add('dark-mode'); const size = localStorage.getItem('acc-size'); if(size) { this.currentSize = parseInt(size); this.applySize(); } },
    render: function() { document.getElementById('dashboard-view').style.display = 'none'; const div = document.getElementById('dynamic-view'); div.style.display = 'block'; div.innerHTML = `<div class="p-4 animate__animated animate__fadeIn"><div class="card p-4 border-0 shadow-sm rounded-4"><h5 class="fw-bold mb-4">Ajustes Visuales</h5><div class="row g-3"><div class="col-md-4"><button onclick="Acc.darkMode()" class="btn btn-outline-dark w-100 p-3 rounded-4">Modo Oscuro</button></div><div class="col-md-4"><button onclick="Acc.changeFont(1)" class="btn btn-outline-primary w-100 p-3 rounded-4">Aumentar Texto</button></div><div class="col-md-4"><button onclick="Acc.changeFont(-1)" class="btn btn-outline-primary w-100 p-3 rounded-4">Disminuir Texto</button></div><div class="col-12 text-center mt-3"><button onclick="Acc.reset()" class="btn btn-danger rounded-pill px-5">Restablecer Todo</button></div></div></div></div>`; },
