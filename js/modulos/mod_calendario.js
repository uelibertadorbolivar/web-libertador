 ...
 * ACTUALIZADO: FORMULARIO CAMALEÓNICO (ADMINISTRATIVO VS ESCOLAR) Y CALCULADORAS
 */
window.ModCalendario = {
    ...
    colectivosList: [], personalList: [], // Listas para el modo administrativo
    ...
    obtenerColorEvento: function(tipo) {
        ...
        if (["Colectivo", "Coordinación", "Brigada", "Comité", "Docente Particular"].includes(tipo)) return { css: 'bg-dark text-white', style: '', rgb: [33, 37, 41] }; // Admin tags
        return { css: 'bg-primary text-white', style: '', rgb: [37, 99, 235] }; 
    },

    dibujarDashboardTarjetas: function() {
        let pVer = window.Aplicacion.permiso('Calendario Escolar', 'ver');
        ...
        let html = estilos + 
            crearTarjeta('Calendario Oficial MPPE', 'Efemérides y actos cívicos.', 'bi-calendar2-heart', "window.ModCalendario.abrirVisorSeguro('Escolar')", cAzul, pVer, 'col-md-6 col-xl-3') + 
            crearTarjeta('Calendario Administrativo', 'Reuniones y gestión.', 'bi-briefcase', "window.ModCalendario.abrirVisorSeguro('Administrativo')", cNara, pVer, 'col-md-6 col-xl-3') + 
            crearTarjeta('Calendario Pedagógico', 'Evaluaciones y entregas.', 'bi-journal-bookmark-fill', "window.ModCalendario.abrirVisorSeguro('Pedagógico')", cVerd, pVer, 'col-md-6 col-xl-3') + 
            crearTarjeta('Planificador', 'Crear o editar actividades.', 'bi-calendar-plus-fill', "window.ModCalendario.abrirPlanificadorConFecha()", cMora, pVer, 'col-md-6 col-xl-3');
        
        document.getElementById('calendario-dashboard').innerHTML = html;
        if(!window.Aplicacion.permiso('Calendario Escolar', 'crear')) { document.getElementById('form-planificador-area').innerHTML = `<div class="text-center py-5"><i class="bi bi-lock text-danger fs-1"></i><h5 class="mt-3 text-muted">No tiene permisos.</h5></div>`; }
    },
    ...
    abrirVisorSeguro: function(tipo) {
        if(!window.Aplicacion.permiso('Calendario Escolar', 'ver')) return;
    ...
    // ✨ MOTOR DEL FORMULARIO CAMALEÓNICO ✨
    toggleForm: function() {
    ...
    calcularDias: function() {
        ...
        if(diasAct <= 0) diasAct = 1; // Mínimo 1 día de actividad
    ...
    abrirPlanificadorConFecha: function(fechaStr = null) { 
        if(!window.Aplicacion.permiso('Calendario Escolar', 'crear')) return Swal.fire('Acceso Denegado', 'Sin permisos.', 'warning');
        ...
        this.toggleForm(); // Forzar el diseño correcto
    ...
    cargarEventos: function(silencioso = false) { 
        if(!silencioso) window.Aplicacion.mostrarCarga(); 
        window.Aplicacion.peticion({ action: "get_calendar" }, (resCal) => { 
            if (resCal && resCal.status === "success") { 
                this.eventos = resCal.eventos || []; 
                this.periodoActivo = resCal.periodoActivo; 
                document.getElementById('lbl-periodo-activo').innerText = this.periodoActivo; 
                
                // ✨ SEGUNDA PETICIÓN: Cargamos los datos del módulo de Colectivos para enlazar el Admin ✨
    ...
    mostrarDetalleEvento: function(id) {
        ...
        let pEditar = window.Aplicacion.permiso('Calendario Escolar', 'editar');
        ...
        // ✨ VISOR DINÁMICO (Admin vs Escolar) ✨
        let badgeHtml = '';
        let infoHtml = '';
        ...
        Swal.fire({ 
            ...
            showCloseButton: true, showCancelButton: pEditar, showConfirmButton: pEditar, confirmButtonText: '<i class="bi bi-pencil me-1"></i> Editar', cancelButtonText: '<i class="bi bi-trash me-1"></i> Borrar', confirmButtonColor: '#0066FF', cancelButtonColor: '#dc3545' 
    ...
    dibujarLista: function() { 
        ...
        let pE = window.Aplicacion.permiso('Calendario Escolar', 'editar'); let pL = window.Aplicacion.permiso('Calendario Escolar', 'eliminar');
        ...
        ePag.forEach(e => {
            ...
            let bE = pE ? `<button class="btn btn-sm btn-light border text-primary me-1 shadow-sm" onclick="window.ModCalendario.editarEvento('${e.id}')"><i class="bi bi-pencil"></i></button>` : ''; let bL = pL ? `<button class="btn btn-sm btn-light border text-danger shadow-sm" onclick="window.ModCalendario.eliminarEvento('${e.id}')"><i class="bi bi-trash"></i></button>` : '';
    ...
    // ✨ MOTOR DE GUARDADO CON LA NUEVA LÓGICA ✨
    guardarEvento: function() { 
        ...
        if(tC === 'Administrativo') {
            tE = document.getElementById('cal-admin-cat').value; // Ej: Brigada
            doc = document.getElementById('cal-admin-entidad').value; // Ej: Brigada de Salud
            ...
            if(!tE || !doc) return Swal.fire('Aviso', 'Debe seleccionar quién programa o coordina esta actividad administrativa.', 'warning');
        } else {
        ...
        let p = { 
            action:'save_calendar', periodo: this.periodoActivo, 
            tipo_calendario: tC, tipo_evento: tE, titulo: tit, 
            inicio: ini, fin: fin, desc: desc,
            objetivo: obj, docentes: doc, requerimientos: req, observacion: obs
        }; 
        
        if(this.editandoId) p.id = this.editandoId; 
    ...
    eliminarEvento: function(id) { Swal.fire({title:'¿Borrar?', icon:'warning', showCancelButton:true, confirmButtonColor:'#dc3545', confirmButtonText:'Borrar'}).then(r=>{ if(r.isConfirmed){ window.Aplicacion.peticion({action:'delete_calendar', id:id}, ()=>this.cargarEventos(true)); }}); },
    ...
    editarEvento: function(id) { 
        ...
        if(tC === 'Administrativo') {
            ...
            document.getElementById('cal-admin-entidad').value = e.docentes; // El Responsable Específico
    ...
    procesarYEnviarCSV: function(textoCSV) { /* ...Código CSV... */ },
    descargarPlantillaCSV: function() { /* ...Plantilla... */ },
    ...
    // ✨ GENERADOR PDF CON COLORES ACTUALIZADOS ✨
