...
const ModDocentes = {
    ...
    cargaTemp:[], // Guarda la data de la grilla que se está editando

    // ==========================================
    // 1. ASIGNAR CARGA ACADÉMICA
    // ==========================================
    initCargaAcademica: function() {
        // Carga masiva de datos necesarios
    ...
    caDibujarGrid: function() {
        ...
        // Obtener el Horario Plantilla del Nivel
        ...
        // Cargar datos guardados del backend
    ...
    caGuardar: function() {
        ...
        Aplicacion.peticion({ action: "save_carga_seccion", grado_id: gId, seccion: sec, cargas: this.cargaTemp }, (res) => {
            if(res.status === 'success') Swal.fire({toast:true, position:'top-end', icon:'success', title:'Horario Guardado', showConfirmButton:false, timer:1500});
        });
    },
    ...
    initVerHorario: function() {
        ...
        Aplicacion.peticion({ action: "get_carga_docente", docente_cedula: Aplicacion.usuario.cedula }, (res) => {
            ...
            // Encontrar todos los bloques de tiempo únicos del docente
            let bloquesUnicos =[];
            cargas.forEach(c => {
    ...
    // ==========================================
    // 3. ASIGNAR CARGOS (Directivos/Admin) - Mantenido intacto
    // ==========================================
    ...
    guardarAsignacion: function() {
        ...
        Aplicacion.peticion({ action: "save_staff_assignment", cedula: sP.value, nombre: sP.options[sP.selectedIndex].getAttribute('data-nombre'), cargoId: sC.value, cargoNombre: sC.options[sC.selectedIndex].getAttribute('data-nombre') }, (res) => { if(res.status === 'success') { Swal.fire('Asignado', '', 'success'); this.initAsignar(); } });
    },
    eliminarAsignacion: function(cedula) {
        Swal.fire({ title: '¿Liberar Cargo?', text: "Saldrá del organigrama.", icon: 'warning', showCancelButton: true, confirmButtonColor: '#FF3D00' }).then(r => { if(r.isConfirmed) Aplicacion.peticion({ action: "delete_staff_assignment", cedula }, (res) => { if(res.status === 'success') this.initAsignar(); }); });
    }
};

// CORRECCIÓN: Se usa la notación de corchetes para evitar errores con las tildes en nombres de variables globales
