...
window.ModGestionExpedientes = {
    ...
    promocionCount: 0, evaluacionCount: 0, // <-- NUEVOS CONTADORES
    expedienteId: null, 
    ...
    init: function() {
        if (!window.Aplicacion.permiso('Gestor de Expedientes', 'ver')) {
    ...
    // ✨ MOTOR DE GENERACIÓN PDF (INYECCIÓN DE TODOS LOS CAMPOS) ✨
    imprimirFichaTecnica: async function() {
        ...
        try {
            ...
            let df = exp.datos_ficha || {};
            let cv = exp.curriculum_vitae || {};
            ...
            const setField = (fieldName, text) => {
                try {
                    ...
                    if (field) {
                        field.setText(text ? String(text) : '');
                        field.setFontSize(10); 
                    }
                } catch(e) { console.log("Campo no encontrado en el PDF:", fieldName); }
            };
            ...
            // Aplanar el PDF para evitar edición
            form.flatten();
            ...
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'PDF Generado y Descargado', showConfirmButton: false, timer: 3000 });

        } catch (error) {
            window.Aplicacion.ocultarCarga();
            console.error("Error generando PDF:", error);
            Swal.fire('Error de Plantilla', 'No se pudo encontrar el archivo "Formato_Ficha.pdf" en la carpeta assets/plantillas/ o la librería pdf-lib no ha cargado correctamente.', 'error');
        }
    },

    // --- MÉTODOS DE UI Y MAPA ---
    ...
    toggleSolicitudUnica: function(selectEl) {
        ...
        if(selectEl.value !== '') { 
            ...
            selEst.classList.remove('text-secondary');
            selEst.classList.add('text-dark');
        } else { 
            selEst.disabled = true; 
            selEst.value = ''; 
            selEst.classList.add('text-secondary');
            selEst.classList.remove('text-dark');
        }
    },

    // ✨ LÓGICA DE INTERFAZ DEL TRABAJADOR Y FAMILIAR ✨
    cambioNeuroTrabajador: function(selectEl) {
        let divConapdis = document.getElementById('div-doc-conapdis');
        let selConapdis = document.getElementById('doc-conapdis');
        if (selectEl.value === 'Neurodivergente') {
            divConapdis.style.display = 'block';
        } else {
            divConapdis.style.display = 'none';
            selConapdis.value = 'No';
        }
    },
    cambioNeuroFamiliar: function(selectElement) {
        let contenedor = selectElement.closest('.familiar-item');
        let divConapdis = contenedor.querySelector('.div-fam-conapdis');
        let selConapdis = contenedor.querySelector('.input-fam-conapdis');
        if (selectElement.value === 'Neurodivergente') {
            divConapdis.style.display = 'block';
        } else {
            divConapdis.style.display = 'none';
            selConapdis.value = 'No';
    ...
    cambioParentescoFamiliar: function(selectElement) {
        let contenedor = selectElement.closest('.familiar-item'); 
        let divConyuge = contenedor.querySelector('.div-fam-conyuge');
        let estatusInput = contenedor.querySelector('.input-fam-estatus-pdvsa');
        let val = selectElement.value;
        if (val === 'Esposo(a)' || val === 'Concubino(a)') {
            divConyuge.style.display = 'block';
        } else {
            divConyuge.style.display = 'none';
            estatusInput.value = 'Nunca ha trabajado en PDVSA'; 
        }
    },
    calcularEdadFamiliar: function(el) {
        let c = el.closest('.familiar-item'); let iE = c.querySelector('.input-fam-edad');
        if (el.value) { let h = new Date(); let n = new Date(el.value); let e = h.getFullYear() - n.getFullYear(); let m = h.getMonth() - n.getMonth(); if (m < 0 || (m === 0 && h.getDate() < n.getDate())) { e--; } iE.value = e >= 0 ? e + ' Años' : '0 Años'; } else { iE.value = ''; }
    },

    agregarFamiliar: function(datos = null) {
        this.familiarCount++; 
        const c = document.getElementById('contenedor-familiares-dinamicos'); document.getElementById('msg-sin-familiares').style.display = 'none';
        let cd = datos ? datos.cedula||'' : ''; let n = datos ? datos.nombres||'' : ''; let p = datos ? datos.parentesco||'' : ''; let neuro = datos ? datos.condicion_neuro||'Neurotípico' : 'Neurotípico'; 
        let conapdis = datos ? datos.conapdis||'No' : 'No'; let vive = datos ? datos.vive_con_trabajador||'Sí' : 'Sí'; let estatus = datos ? datos.estatus_pdvsa||'Nunca ha trabajado en PDVSA' : 'Nunca ha trabajado en PDVSA';
        let f = datos ? datos.fecha_nacimiento||'' : ''; let e = datos ? datos.edad||'' : ''; let es = datos ? datos.estudiante_de||'' : '';
        
        let displayConyuge = (p === 'Esposo(a)' || p === 'Concubino(a)') ? 'block' : 'none';
        let displayConapdis = (neuro === 'Neurodivergente') ? 'block' : 'none';

        let h = `
        <div class="caja-dinamica familiar-item animate__animated animate__fadeIn border-warning" style="background-color: #fffbeb;" id="fam-${this.familiarCount}">
            <button type="button" class="btn btn-danger btn-eliminar-item" onclick="window.ModGestionExpedientes.eliminarGenerico('fam-${this.familiarCount}', 'familiar-item', 'msg-sin-familiares')"><i class="bi bi-x-lg"></i></button>
            <div class="row g-3">
                <div class="col-md-2"><label class="form-label label-blue small mb-1">Cédula o C.E.</label><input type="text" class="input-moderno input-fam-cedula" value="${cd}"></div>
                <div class="col-md-5"><label class="form-label label-blue small mb-1">Nombres y Apellidos</label><input type="text" class="input-moderno input-fam-nombres txt-format" value="${n}"></div>
                <div class="col-md-3"><label class="form-label label-blue small mb-1">Parentesco</label><select class="input-moderno input-fam-parentesco" onchange="window.ModGestionExpedientes.cambioParentescoFamiliar(this)"><option value="">Sel...</option><option value="Hijo(a)" ${p==='Hijo(a)'?'selected':''}>Hijo(a)</option><option value="Esposo(a)" ${p==='Esposo(a)'?'selected':''}>Esposo(a)</option><option value="Concubino(a)" ${p==='Concubino(a)'?'selected':''}>Concubino(a)</option><option value="Padre" ${p==='Padre'?'selected':''}>Padre</option><option value="Madre" ${p==='Madre'?'selected':''}>Madre</option></select></div>
                <div class="col-md-2"><label class="form-label label-blue small mb-1">¿Vive con usted?</label><select class="input-moderno input-fam-vive fw-bold text-primary"><option value="Sí" ${vive==='Sí'?'selected':''}>Sí</option><option value="No" ${vive==='No'?'selected':''}>No</option></select></div>
                
                <div class="col-md-3"><label class="form-label label-blue small mb-1">Condición Neurológica</label><select class="input-moderno input-fam-neuro" onchange="window.ModGestionExpedientes.cambioNeuroFamiliar(this)"><option value="Neurotípico" ${neuro==='Neurotípico'?'selected':''}>Neurotípico</option><option value="Neurodivergente" ${neuro==='Neurodivergente'?'selected':''}>Neurodivergente</option></select></div>
                <div class="col-md-2 div-fam-conapdis" style="display: ${displayConapdis};"><label class="form-label label-blue small mb-1">Cert. CONAPDIS</label><select class="input-moderno input-fam-conapdis fw-bold text-danger"><option value="No" ${conapdis==='No'?'selected':''}>No</option><option value="Sí" ${conapdis==='Sí'?'selected':''}>Sí posee</option></select></div>
                
                <div class="col-md-3"><label class="form-label label-blue small mb-1">Fecha Nac.</label><input type="date" class="input-moderno input-fam-fecha" value="${f}" onchange="window.ModGestionExpedientes.calcularEdadFamiliar(this)"></div>
                <div class="col-md-1"><label class="form-label label-blue small mb-1">Edad</label><input type="text" class="input-moderno input-fam-edad bg-light text-center fw-bold px-1" readonly value="${e}"></div>
                ...
                <div class="col-md-12 div-fam-conyuge mt-2" style="display: ${displayConyuge};">
                    <div class="p-2 bg-info bg-opacity-10 border border-info rounded d-flex align-items-center">
                        <i class="bi bi-briefcase-fill text-info fs-4 me-3"></i>
                        <div class="flex-grow-1">
                            <label class="form-label text-dark fw-bold mb-1">Estatus Laboral del Cónyuge/Concubino(a) respecto a PDVSA:</label>
                            <select class="input-moderno input-fam-estatus-pdvsa border-info">
                                <option value="Nunca ha trabajado en PDVSA" ${estatus==='Nunca ha trabajado en PDVSA'?'selected':''}>Nunca ha trabajado en PDVSA</option>
                                <option value="Trabajador(a) Activo(a)" ${estatus==='Trabajador(a) Activo(a)'?'selected':''}>Trabajador(a) Activo(a)</option>
                                <option value="Trabajó / Está Retirado(a)" ${estatus==='Trabajó / Está Retirado(a)'?'selected':''}>Trabajó / Está Retirado(a)</option>
                            </select>
                        </div>
                    </div>
                </div>
    ...
    agregarPosicion: function(datos = null) {
        ...
        let h = `<div class="caja-dinamica posicion-item animate__animated animate__fadeIn" id="pos-${this.posicionCount}"><button type="button" class="btn btn-danger btn-eliminar-item" onclick="window.ModGestionExpedientes.eliminarGenerico('pos-${this.posicionCount}', 'posicion-item', 'msg-sin-posiciones')"><i class="bi bi-x-lg"></i></button>
            <div class="row g-3"><div class="col-md-2"><label class="form-label label-blue small mb-1">Año</label><input type="number" class="input-moderno input-pos-anio" value="${an}"></div><div class="col-md-4"><label class="form-label label-blue small mb-1">Organización</label><select class="input-moderno input-pos-org">${ho}</select></div><div class="col-md-3"><label class="form-label label-blue small mb-1">Negocio</label><select class="input-moderno input-pos-neg">${hn}</select></div><div class="col-md-3"><label class="form-label label-blue small mb-1">Filial</label><input type="text" class="input-moderno input-pos-filial txt-format" value="${fi}"></div></div></div>`;
    ...
    agregarExperiencia: function(datos = null) {
        ...
        let h = `<div class="caja-dinamica experiencia-item animate__animated animate__fadeIn" id="exp-${this.experienciaCount}"><button type="button" class="btn btn-danger btn-eliminar-item" onclick="window.ModGestionExpedientes.eliminarGenerico('exp-${this.experienciaCount}', 'experiencia-item', 'msg-sin-experiencias')"><i class="bi bi-x-lg"></i></button>
            <div class="row g-3"><div class="col-md-2"><label class="form-label label-blue small mb-1">Año</label><input type="number" class="input-moderno input-exp-anio" value="${an}"></div><div class="col-md-5"><label class="form-label label-blue small mb-1">Empresa</label><input type="text" class="input-moderno input-exp-emp txt-format" value="${em}"></div><div class="col-md-5"><label class="form-label label-blue small mb-1">Cargo</label><input type="text" class="input-moderno input-exp-car txt-format" value="${ca}"></div></div></div>`;
    ...
    guardarExpediente: async function() {
        if (!window.Aplicacion.permiso('Gestor de Expedientes', 'modificar')) {
            Swal.fire('Denegado', 'Solo puedes visualizar los expedientes, no modificarlos.', 'error');
            return;
        }
        ...
        try {
            let errorBD;
            if (this.expedienteId) { const { error } = await window.supabaseDB.from('expedientes_docentes').update(payload).eq('id', this.expedienteId); errorBD = error; } 
            else { const { data, error } = await window.supabaseDB.from('expedientes_docentes').insert([payload]).select(); errorBD = error; if (data && data.length > 0) this.expedienteId = data[0].id; }
            
            this.expedienteDataCache = payload; 
            
            window.Aplicacion.ocultarCarga(); if (errorBD) throw errorBD;
            window.Aplicacion.auditar('Expediente Docente', 'Actualización de Administrador', `El admin auditó la ficha de la CI: ${cedula}`);
            Swal.fire({ title: '¡Expediente Guardado!', text: 'Se actualizó la ficha del docente.', icon: 'success', confirmButtonColor: '#f59e0b' });
        } catch (e) { window.Aplicacion.ocultarCarga(); Swal.fire('Error', 'Hubo un problema al guardar su expediente.', 'error'); }
    },
    ...
    cargarExpediente: async function(cedulaBuscar) {
        try {
            ...
            if (data && data.length > 0) {
                ...
                if (exp.planificacion_estrategica) {
