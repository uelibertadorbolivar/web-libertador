...
window.ModExpedienteDocente = {
    ...
    agregarPosicion: function(datos = null) {
        ...
        let h = `<div class="caja-dinamica posicion-item animate__animated animate__fadeIn" id="pos-${this.posicionCount}"><button type="button" class="btn btn-danger btn-eliminar-item" onclick="window.ModExpedienteDocente.eliminarGenerico('pos-${this.posicionCount}', 'posicion-item', 'msg-sin-posiciones')"><i class="bi bi-x-lg"></i></button>
    ...
    // ✨ MOTOR CONSTRUCTOR DE CARGA DE DATOS ✨
    ...
    // ✨ MOTOR DE AUTOGUARDADO SILENCIOSO ✨
    autoGuardarSilencioso: async function() {
        ...
        let payload = this.obtenerPayloadActual(false); // false = es un borrador
        ...
        try {
            ...
            // Memoria de UX: Guardamos el paso en el que se quedó
    ...
    // ✨ GUARDADO MANUAL (Botón Finalizar) ✨
    guardarExpediente: async function() {
        let payload = this.obtenerPayloadActual(true); // true = finalizado oficial
        window.Aplicacion.mostrarCarga();
    ...
    cargarExpediente: async function(cedulaBuscar) {
        try {
            ...
            if (data && data.length > 0) {
                ...
                // Función para decodificar JSON a prueba de balas
                ...
                // ✨ MEMORIA DE UX: Llevar al docente a la pestaña donde se quedó
