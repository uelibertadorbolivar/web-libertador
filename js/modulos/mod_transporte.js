 ...
 * ACTUALIZADO: BOTÓN REFRESACAR EN VISOR Y FIN DE RUTA
 */
...
window.ModTransporte = {
    ...
    cargarTodo: function(silencioso = false) {
        ...
        window.Aplicacion.peticion({ action: "get_transporte_data", fecha_hoy: this.obtenerFechaHoy() }, (res) => {
            ...
            if (res && res.status === "success") {
                ...
                if(this.vistaActualOculta === 'Visor') this.reRenderVisorSilencioso(); // ✨ RE-RENDER ESPECIAL ✨
            }
        });
    },
    ...
    llenarSelectores: function() {
        ...
        let paradasOcupadas = [];
        this.rutas.forEach(r => { if(r.id_ruta !== this.editandoRutaId) { try { paradasOcupadas = paradasOcupadas.concat(JSON.parse(r.paradas_json || "[]")); } catch(e){} } });
    ...
    verDetalleRuta: function(idRuta) { let r = this.rutas.find(x => x.id_ruta === idRuta); if(!r) return; let doc = this.docentes.find(d => String(d.cedula) === String(r.cedula_docente)); let nombreDoc = doc ? (doc.nombre_completo || doc.nombre) : 'Sin asignar'; let telDoc = doc && doc.telefono ? doc.telefono : 'No registrado'; let idArr = []; try { idArr = JSON.parse(r.paradas_json || "[]"); } catch(e){} let html = `<div class="text-start"><div class="mb-4 text-center"><span class="badge bg-warning text-dark shadow-sm me-2 py-2 px-3 mb-2"><i class="bi bi-person-vcard me-1"></i> Chofer: ${r.chofer}</span><span class="badge bg-primary shadow-sm py-2 px-3 mb-2"><i class="bi bi-person-video3 me-1"></i> Guía: ${nombreDoc}</span><div class="mt-2 text-muted small"><i class="bi bi-telephone-fill me-1 text-success"></i> Teléfono del Guía: <span class="fw-bold">${telDoc}</span></div></div><div class="timeline-rutograma mt-3 pt-2"><div class="timeline-node"><div class="timeline-icon start"><i class="bi bi-bus-front-fill"></i></div><div class="timeline-content border-warning border-2 shadow-sm"><span class="fw-bold text-dark mb-0">Inicio del Recorrido</span></div></div>`; idArr.forEach((pid, idx) => { let p = this.paradas.find(x => x.id_parada === pid); if(p) { html += `<div class="timeline-node"><div class="timeline-icon stop"><i class="bi bi-signpost-fill"></i></div><div class="timeline-content shadow-sm"><div><span class="badge bg-info text-dark me-2">${idx+1}</span><span class="fw-bold text-dark">${p.nombre_parada}</span></div>${p.referencia ? `<div class="small text-muted fst-italic ms-2">${p.referencia}</div>` : ''}</div></div>`; } }); html += `<div class="timeline-node"><div class="timeline-icon end"><i class="bi bi-building-fill"></i></div><div class="timeline-content border-success border-2 shadow-sm"><span class="fw-bold text-success mb-0">Llegada U.E. Libertador Bolívar</span></div></div></div></div>`; Swal.fire({ title: `<div class="text-primary fw-bold fs-3"><i class="bi bi-map-fill me-2"></i>${r.nombre_ruta}</div>`, html: html, confirmButtonText: 'Cerrar Visor', confirmButtonColor: '#0066FF', width: '600px' }); },
    ...
    editarRuta: function(id) { let r = this.rutas.find(x => x.id_ruta === id); if(!r) return; this.editandoRutaId = id; document.getElementById('txt-ruta-nombre').value = r.nombre_ruta; document.getElementById('txt-ruta-chofer').value = r.chofer; this.paradasTemporalesRuta = []; try { let idArr = JSON.parse(r.paradas_json || "[]"); idArr.forEach(pid => { let pObj = this.paradas.find(p => p.id_parada === pid); if(pObj) this.paradasTemporalesRuta.push(pObj); }); } catch(e){} this.llenarSelectores(); document.getElementById('sel-ruta-docente').value = r.cedula_docente || ""; this.dibujarRutogramaVivo(); document.getElementById('btn-guardar-ruta').innerText = "Actualizar Ruta"; document.getElementById('btn-cancelar-ruta').classList.remove('d-none'); },
    ...
    dibujarTarjetasRutas: function() {
        ...
        this.rutas.forEach(r => {
            let doc = this.docentes.find(d => String(d.cedula) === String(r.cedula_docente)); let idArr = []; try { idArr = JSON.parse(r.paradas_json); } catch(e){}
    ...
    // ✨ FUNCIÓN DE REFRESCO MANUAL (MUESTRA PANTALLA DE CARGA) ✨
    refrescarVisor: function() {
        let idRuta = document.getElementById('vis-sel-ruta').value;
        if(!idRuta) return; // Nada que refrescar
        this.cargarTodo(false); // Fals indica que NO es silencioso, muestra pantalla de carga
    },

    // ✨ FUNCIÓN ESPECIAL PARA RE-DIBUJAR SIN TOCAR LOS SELECTS ✨
    reRenderVisorSilencioso: function() {
        let divArea = document.getElementById('vis-recorrido-area');
        if(!divArea || divArea.classList.contains('d-none')) return; // No está activo
    ...
    renderizarLineaTiempo: function(idSelectRuta, idSelectMomento, idDivArea, esOperacion) {
        ...
        let marcadas = {}; if(trackActual) { try { marcadas = JSON.parse(trackActual.estado_json); } catch(e){} }
        let rutaIds = []; try { rutaIds = JSON.parse(ruta.paradas_json); } catch(e){}
        let puntosRecorrido = [];
