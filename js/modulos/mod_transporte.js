 ...
 * ACTUALIZADO: REORDENAMIENTO DINÁMICO DE RUTAS (PLANIFICACIÓN Y OPERACIÓN)
 */
...
window.ModTransporte = {
    ...
    // ==========================================
    // SELECTORES Y PLANIFICACIÓN DE RUTAS
    // ==========================================
    llenarSelectores: function() {
        ...
        this.rutas.forEach(r => { 
            if(r.id_ruta !== this.editandoRutaId) { 
                try { let arr = typeof r.paradas_json === 'string' ? JSON.parse(r.paradas_json || "[]") : (r.paradas_json || []); paradasOcupadas = paradasOcupadas.concat(arr); } catch(e){} 
            } 
        });
    ...
    // ✨ MOVER PARADAS EN LA PLANIFICACIÓN ✨
    moverParadaTemporal: function(idx, dir) {
        let nIdx = idx + dir;
        if(nIdx < 0 || nIdx >= this.paradasTemporalesRuta.length) return;
        let tmp = this.paradasTemporalesRuta[idx];
        this.paradasTemporalesRuta[idx] = this.paradasTemporalesRuta[nIdx];
        this.paradasTemporalesRuta[nIdx] = tmp;
        this.dibujarRutogramaVivo();
    },

    dibujarRutogramaVivo: function() { 
        let visor = document.getElementById('visor-rutograma-vivo'); if(!visor) return; 
        if(this.paradasTemporalesRuta.length === 0) { visor.innerHTML = '<div class="text-center text-muted small border rounded-3 p-4 bg-light shadow-sm">Añada paradas en la parte superior.</div>'; return; } 
    ...
    // ✨ MOVER PARADAS DURANTE LA OPERACIÓN (CERTIFICAR/REORDENAR DIARIAMENTE) ✨
    moverParadaOperacion: function(idRuta, tipo, index, direccion) {
        let trackActual = this.trackingHoy.find(t => t.id_ruta === idRuta && t.tipo_recorrido === tipo);
        let marcadas = {};
        if(trackActual) { try { marcadas = typeof trackActual.estado_json === 'string' ? JSON.parse(trackActual.estado_json || "{}") : (trackActual.estado_json || {}); } catch(e){} }
        ...
        let defaultOrder = [];
        if (tipo === 'Ida') {
            rutaIds.forEach(id => { let p = this.paradas.find(x => x.id_parada === id); if(p) defaultOrder.push(p.id_parada); });
            defaultOrder.push('escuela');
        } else {
            defaultOrder.push('escuela');
            let reversa = [...rutaIds].reverse();
            reversa.forEach(id => { let p = this.paradas.find(x => x.id_parada === id); if(p) defaultOrder.push(p.id_parada); });
        }
        ...
        if (nuevoIndex < 0 || nuevoIndex >= ordenActual.length) return;
        
        // Reglas estrictas: No se puede mover el punto de la escuela
        if (tipo === 'Ida' && (index === ordenActual.length - 1 || nuevoIndex === ordenActual.length - 1)) return; 
        if (tipo === 'Retorno' && (index === 0 || nuevoIndex === 0)) return; 
        
        let temp = ordenActual[index];
        ordenActual[index] = ordenActual[nuevoIndex];
        ordenActual[nuevoIndex] = temp;
        ...
        window.Aplicacion.peticion({ action: 'update_tracking', fecha_str: this.obtenerFechaHoy(), id_ruta: idRuta, tipo_recorrido: tipo, estado_json: marcadas }, (res) => {
            ...
            if(res && res.status === 'success') { this.cargarTodo(true); }
        });
    },
    ...
    renderizarLineaTiempo: function(idSelectRuta, idSelectMomento, idDivArea, esOperacion) {
        ...
        // Construir arreglo de objetos de la base
        let defaultPuntos = [];
        if (tipo === 'Ida') {
            rutaIds.forEach(id => { let p = this.paradas.find(x => x.id_parada === id); if(p) defaultPuntos.push({id: p.id_parada, nombre: p.nombre_parada, ref: p.referencia, tipo: 'parada'}); });
            defaultPuntos.push({id: 'escuela', nombre: 'U.E. Libertador Bolívar', ref: 'Llegada a la Institución', tipo: 'llegada'});
        } else {
            defaultPuntos.push({id: 'escuela', nombre: 'U.E. Libertador Bolívar', ref: 'Salida de la Institución', tipo: 'salida'});
            let reversa = [...rutaIds].reverse();
            reversa.forEach(id => { let p = this.paradas.find(x => x.id_parada === id); if(p) defaultPuntos.push({id: p.id_parada, nombre: p.nombre_parada, ref: p.referencia, tipo: 'parada'}); });
        }

        // Aplicar orden modificado dinámicamente si existe
        let puntosRecorrido = [];
        if (marcadas._orden && Array.isArray(marcadas._orden) && marcadas._orden.length === defaultPuntos.length) {
            puntosRecorrido = marcadas._orden.map(id => defaultPuntos.find(p => p.id === id)).filter(p => p);
            if (puntosRecorrido.length !== defaultPuntos.length) puntosRecorrido = defaultPuntos;
        } else {
            puntosRecorrido = defaultPuntos;
        }
        ...
        puntosRecorrido.forEach((pto, idx) => {
            ...
            let botonMarcar = '';
            let botonesReordenar = '';

            if (puedeMarcar && !horaPaso && esOperacion) {
                let textoBtn = pto.tipo === 'salida' ? 'Marcar Salida' : (pto.tipo === 'llegada' ? 'Marcar Llegada' : 'Pasó el Bus');
                botonMarcar = `<button class="btn btn-sm btn-info text-white fw-bold shadow-sm rounded-pill px-3 ms-auto hover-efecto" onclick="window.ModTransporte.preguntarHoraCheckPoint('${idRuta}', '${tipo}', '${pto.id}')"><i class="bi bi-clock-history me-1"></i> <span class="d-none d-sm-inline">${textoBtn}</span><span class="d-sm-none">Marcar</span></button>`;
                
                // Restricciones lógicas para los botones de reordenamiento
                let disableUp = false; let disableDown = false;
                if (tipo === 'Ida') {
                    if (idx === puntosRecorrido.length - 1) { disableUp = true; disableDown = true; } // Escuela intocable al final
                    if (idx === 0) disableUp = true;
                    if (idx === puntosRecorrido.length - 2) disableDown = true; // No bajar más allá de la escuela
                } else {
                    if (idx === 0) { disableUp = true; disableDown = true; } // Escuela intocable al inicio
                    if (idx === 1) disableUp = true; // No subir más arriba de la escuela
                    if (idx === puntosRecorrido.length - 1) disableDown = true;
                }

                if(pto.id !== 'escuela') {
                    botonesReordenar = `
                        <div class="d-flex flex-column ms-1 me-3 justify-content-center align-items-center">
                            <button class="btn btn-sm text-secondary p-0 hover-efecto" style="line-height: 0.5; ${disableUp ? 'opacity:0.2; cursor:not-allowed;' : ''}" ${disableUp ? 'disabled' : ''} onclick="window.ModTransporte.moverParadaOperacion('${idRuta}', '${tipo}', ${idx}, -1)" title="Subir parada"><i class="bi bi-caret-up-fill fs-4"></i></button>
                            <button class="btn btn-sm text-secondary p-0 hover-efecto mt-1" style="line-height: 0.5; ${disableDown ? 'opacity:0.2; cursor:not-allowed;' : ''}" ${disableDown ? 'disabled' : ''} onclick="window.ModTransporte.moverParadaOperacion('${idRuta}', '${tipo}', ${idx}, 1)" title="Bajar parada"><i class="bi bi-caret-down-fill fs-4"></i></button>
                        </div>
                    `;
                }
            }
            ...
            htmlTimeline += `
                <div class="timeline-node ${estadoClase} animate__animated animate__fadeInLeft position-relative">
                    ...
                    <div class="timeline-content ${horaPaso ? 'border-success bg-white' : 'shadow-sm'}">
                        <div class="d-flex align-items-center">
                            ${botonesReordenar}
                            <div>
                                <div class="fw-bold fs-6 text-dark">${pto.nombre} ${badgeHora}</div>
