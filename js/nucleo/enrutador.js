...
const Enrutador = {
    ...
    cargarVista: async function(nombreVista, guardarHistorial = true) {
        ...
        this.vistaActual = nombreVista; document.getElementById('titulo-pagina').innerText = nombreVista; window.Aplicacion.marcarMenuActivo(nombreVista);
        ...
        try {
            if (window.Aplicacion.ModulosSistema && window.Aplicacion.ModulosSistema[nombreVista]) {
                ...
                if (response.ok) { 
                    ...
                    else { console.warn("Falta declarar: " + nombreFuncion); if(typeof window.Aplicacion !== 'undefined') window.Aplicacion.ocultarCarga(); }
