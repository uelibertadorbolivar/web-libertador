 ...
 * MOTOR DE PERMISOS, DASHBOARDS PREMIUM Y BLINDAJE DE RED
 * ACTUALIZADO: AGREGADO MÓDULO "GESTIÓN ESTUDIANTIL"
 */
...
window.ModulosSistema = {
    ...
    // ✨ AQUÍ AGREGAMOS EL NUEVO MÓDULO ESTUDIANTIL ✨
    "Gestión Estudiantil": { 
...
window.Aplicacion = {
    usuario: null, ModulosSistema: window.ModulosSistema, momentoActual: null, rolesDelSistema: [], permisosActuales: {}, 
    ...
    init: function() { 
        ...
        if (guardado) { 
            this.usuario = JSON.parse(guardado); this.prepararApp(); 
        } else { 
            setTimeout(() => { 
                ...
                if(inputCedula) {
                    inputCedula.addEventListener('keypress', function(e) {
                        if (e.key === 'Enter') window.Aplicacion.verificarUsuario();
                    });
                }
                ...
                if(inputClave) {
                    inputClave.addEventListener('keypress', function(e) {
                        if (e.key === 'Enter') window.Aplicacion.iniciarSesion();
                    });
                }
            }, 1000); 
        } 
    ...
    peticion: function(payload, callback) { 
        ...
        .then(data => {
            try { callback(data); } 
            catch(errorVisual) { console.warn("Interrupción de vista al renderizar.", errorVisual); }
        })
        .catch(err => { 
            console.error("Error Real de Conexión:", err); 
            this.ocultarCarga(); 
            if(typeof Swal !== 'undefined') Swal.fire('Error', 'Falla de conexión al servidor.', 'error'); 
        }); 
    },
    ...
    iniciarSesion: function() { const c = document.getElementById('inputCedula').value; const p = document.getElementById('inputClave').value; if (!p) return Swal.fire('Atención', 'Ingrese contraseña.', 'warning'); this.mostrarCarga(); this.peticion({ action: "login", cedula: c, password: p }, (res) => { this.ocultarCarga(); if(res && res.status === "success") { this.usuario = res.user; this.usuario.token = res.token; localStorage.setItem('sigae_usuario', JSON.stringify(this.usuario)); this.prepararApp(); } else { Swal.fire('Atención', res.message || 'Error.', 'error'); } }); },
    ...
    generarDashboardModulo: function(nombreCategoria) { 
        ...
        const estilos = `
        <style>
            .tarjeta-cat { background: #ffffff; border-radius: 20px; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); cursor: pointer; overflow: hidden; position: relative; display: flex; flex-direction: column; }
            .tarjeta-cat:hover { transform: translateY(-8px); box-shadow: 0 15px 35px rgba(0,0,0,0.1) !important; }
            .tarjeta-cat .bg-icono-gigante { position: absolute; right: -20px; bottom: -20px; font-size: 8rem; opacity: 0.03; transition: transform 0.5s ease; }
            .tarjeta-cat:hover .bg-icono-gigante { transform: scale(1.2) rotate(-10deg); }
            .tarjeta-cat .icono-cat { width: 70px; height: 70px; border-radius: 18px; display: flex; align-items: center; justify-content: center; font-size: 2.2rem; margin-bottom: 1.5rem; transition: transform 0.3s ease; }
            .tarjeta-cat:hover .icono-cat { transform: scale(1.1); }
            .banner-padre { border-radius: 24px; position: relative; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.1); }
            
            /* ✨ ANIMACIÓN Y CRISTAL 3D ✨ */
        ...
        modulo.items.forEach((item, index) => { 
            const tieneAcceso = this.permiso(item.vista, 'ver'); 
            const color = paleta[index % paleta.length];
            
            if(tieneAcceso) {
                htmlTarjetas += `
                <div class="col-12 col-md-6 col-xl-4 animate__animated animate__fadeInUp" style="animation-delay: ${index * 0.1}s">
                    <div class="tarjeta-cat p-4 h-100 shadow-sm" style="background: ${color.bg}; border: 1px solid ${color.border};" onclick="Enrutador.navegar('${item.vista}')">
                        <i class="bi ${item.icono} text-dark bg-icono-gigante"></i>
                        <div class="icono-cat shadow-sm" style="color: ${color.text}; background: white; border: 1px solid ${color.border};"><i class="bi ${item.icono}"></i></div>
                        <h4 class="fw-bold text-dark mb-2" style="position: relative; z-index: 2;">${item.vista}</h4>
                        <div class="mt-auto pt-3 d-flex align-items-center fw-bold" style="color: ${color.text}; font-size: 0.9rem; position: relative; z-index: 2;">Entrar al submódulo <i class="bi bi-arrow-right ms-2 transition-transform"></i></div>
                    </div>
                </div>`;
            } else {
                htmlTarjetas += `
                <div class="col-12 col-md-6 col-xl-4 animate__animated animate__fadeInUp" style="animation-delay: ${index * 0.1}s">
                    <div class="tarjeta-cat p-4 h-100 shadow-sm bg-light opacity-75" style="border: 1px solid #e2e8f0; cursor: not-allowed;" onclick="Swal.fire('Acceso Denegado', 'Su rol no tiene permisos para este submódulo.', 'error')">
                        <i class="bi ${item.icono} text-muted bg-icono-gigante"></i>
                        <div class="icono-cat bg-secondary bg-opacity-10 text-secondary border-0"><i class="bi bi-lock-fill"></i></div>
                        <h4 class="fw-bold text-muted mb-2" style="position: relative; z-index: 2;">${item.vista}</h4>
                        <div class="mt-auto pt-3 d-flex align-items-center fw-bold text-danger" style="font-size: 0.9rem; position: relative; z-index: 2;"><i class="bi bi-shield-lock-fill me-2"></i> Acceso Restringido</div>
                    </div>
                </div>`;
            }
        }); 
                ...
                <div class="banner-padre p-4 p-md-5 text-white" style="background: linear-gradient(135deg, ${modulo.color} 0%, rgba(0,0,0,0.4) 150%);">
                    ...
                    <div class="row align-items-center position-relative z-1">
                        <div class="col-md-9 text-center text-md-start mb-3 mb-md-0">
                            <span class="badge bg-white shadow-sm mb-3 px-3 py-2 fw-bold" style="color: ${modulo.color}; letter-spacing: 1px; font-size: 0.85rem;"><i class="bi ${modulo.icono} me-1"></i> CATEGORÍA DEL SISTEMA</span>
