...
window.ModulosSistema = {
    ...
    "Control de Estudios": { 
        ...
        items: [
            { vista: "Grados y Salones", icono: "bi-grid-3x3-gap-fill", roles: ["Directivo", "Administrador", "Control de Estudios"] }, 
    ...
    "Gestión Docente": { 
        ...
        items: [
            // AQUÍ ESTÁ EL NUEVO MÓDULO DE GUIATURAS
            { vista: "Asignar Guiaturas", icono: "bi-person-video3", roles: ["Directivo", "Administrador", "Coordinador", "RRHH"] }, 
...
window.Aplicacion = {
    ...
    peticion: function(payload, callback) { fetch(Configuracion.obtenerApiUrl(), { method: 'POST', body: JSON.stringify(payload) }).then(res => res.json()).then(data => callback(data)).catch(err => { console.error("Error en petición:", err); this.ocultarCarga(); if(typeof Swal !== 'undefined') { Swal.fire('Error de Conexión', 'No se pudo contactar al servidor.', 'error'); } }); },
    ...
    prepararApp: function() {
        ...
        this.peticion({ action: "get_current_moment" }, (res) => {
            ...
            if(this.usuario) { const navNombre = document.getElementById('nombre-usuario-nav'); const navRol = document.getElementById('rol-usuario-nav'); if(navNombre) navNombre.innerText = this.usuario.nombre; if(navRol) navRol.innerText = this.usuario.rol; }
    ...
    verificarUsuario: function() { const cedulaEntrada = document.getElementById('inputCedula').value; if(!cedulaEntrada) return Swal.fire('Atención', 'Debe ingresar su número de cédula.', 'warning'); this.mostrarCarga(); this.peticion({ action: "verificar_usuario", cedula: cedulaEntrada }, (res) => { if(res && res.found) { if(res.requiere_configuracion) { document.getElementById('paso-cedula').style.display = 'none'; document.getElementById('pi-nombre').value = res.nombre; this.cargarPreguntasSeguridad(res.nombre); } else { this.ocultarCarga(); document.getElementById('lbl-nombre-login').innerText = res.nombre; document.getElementById('paso-cedula').style.display = 'none'; document.getElementById('paso-clave').style.display = 'block'; } } else { this.ocultarCarga(); Swal.fire('No encontrado', 'Cédula no registrada.', 'error'); } }); },
    cargarPreguntasSeguridad: function(nombreUsuario) { this.peticion({ action: "get_security_questions" }, (res) => { this.ocultarCarga(); if(res && res.status === "success" && res.preguntas) { let opcionesHTML = '<option value="" disabled selected>-- Seleccione una pregunta --</option>'; res.preguntas.forEach(pregunta => { opcionesHTML += `<option value="${pregunta}">${pregunta}</option>`; }); document.getElementById('pi-preg1').innerHTML = opcionesHTML; document.getElementById('pi-preg2').innerHTML = opcionesHTML; document.getElementById('paso-primer-ingreso').style.display = 'block'; Swal.fire({ toast: true, position: 'top-end', icon: 'info', title: `¡Hola ${nombreUsuario}! Por favor configura tu cuenta.`, showConfirmButton: false, timer: 4000 }); } else { Swal.fire('Error', 'Falla de conexión al cargar preguntas.', 'error'); } }); },
    iniciarSesion: function() { const cedulaEntrada = document.getElementById('inputCedula').value; const claveEntrada = document.getElementById('inputClave').value; if (!claveEntrada) return Swal.fire('Atención', 'Debe ingresar su contraseña.', 'warning'); this.mostrarCarga(); this.peticion({ action: "login", cedula: cedulaEntrada, password: claveEntrada }, (res) => { this.ocultarCarga(); if(res && res.status === "success") { this.usuario = res.user; this.usuario.token = res.token; localStorage.setItem('sigae_usuario', JSON.stringify(this.usuario)); this.prepararApp(); } else if (res && res.status === "clave_vencida") { Swal.fire('Clave Vencida', res.message, 'warning'); } else { Swal.fire('Atención', (res && res.message) ? res.message : 'Error de acceso.', 'error'); } }); },
    ...
    guardarPrimerIngreso: function() { const cedulaUsuario = document.getElementById('inputCedula').value; const nombreUsuario = document.getElementById('pi-nombre').value; const emailUsuario = document.getElementById('pi-email').value; const telefonoUsuario = document.getElementById('pi-telefono').value; const pregunta1 = document.getElementById('pi-preg1').value; const respuesta1 = document.getElementById('pi-resp1').value; const pregunta2 = document.getElementById('pi-preg2').value; const respuesta2 = document.getElementById('pi-resp2').value; const claveNueva1 = document.getElementById('pi-clave1').value; const claveNueva2 = document.getElementById('pi-clave2').value; if(!emailUsuario || !telefonoUsuario || !pregunta1 || !respuesta1 || !pregunta2 || !respuesta2 || !claveNueva1 || !claveNueva2) return Swal.fire('Datos Incompletos', 'Complete todos los datos.', 'warning'); if(claveNueva1 !== claveNueva2) return Swal.fire('Error', 'Las contraseñas no coinciden.', 'error'); if(!/^(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#.])[A-Za-z\d@$!%*?&#.]{8,}$/.test(claveNueva1)) return Swal.fire('Contraseña Débil', 'Mínimo 8 caracteres, 1 mayúscula, 1 número y 1 símbolo.', 'error'); this.mostrarCarga(); this.peticion({ action: "primer_ingreso", cedula: cedulaUsuario, nombre: nombreUsuario, email: emailUsuario, telefono: telefonoUsuario, preg_1: pregunta1, resp_1: respuesta1, preg_2: pregunta2, resp_2: respuesta2, nueva_clave: claveNueva1 }, (res) => { this.ocultarCarga(); if(res && res.status === "success") { Swal.fire('¡Perfil Creado!', 'Cuenta configurada. Inicie sesión.', 'success').then(() => { location.reload(); }); } else { Swal.fire('Error', res ? res.message : 'Error en configuración.', 'error'); } }); },
    ...
    enviarRegistroInvitado: function() { const nombreInv = document.getElementById('inv-nombre').value; const correoInv = document.getElementById('inv-correo').value; const telefonoInv = document.getElementById('inv-telefono').value; const motivoInv = document.getElementById('inv-motivo').value; if(!nombreInv || !correoInv || !telefonoInv || !motivoInv) return Swal.fire('Campos Incompletos', 'Debe llenar todos los datos.', 'warning'); this.mostrarCarga(); this.peticion({ action: "registrar_invitado", nombre: nombreInv, correo: correoInv, telefono: telefonoInv, motivo: motivoInv }, (res) => { this.ocultarCarga(); if(res && res.status === "success") { this.usuario = res.user; this.usuario.token = res.token; localStorage.setItem('sigae_usuario', JSON.stringify(this.usuario)); Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Acceso Invitado', showConfirmButton: false, timer: 2000 }); setTimeout(() => { this.prepararApp(); }, 1000); } else { Swal.fire('Error', 'Falla en el registro.', 'error'); } }); },
    ...
    dibujarMenuPrincipal: function() { 
        ...
        htmlMenu += `</div>`; contenedorEnlaces.innerHTML = htmlMenu; 
    },
    ...
    marcarMenuActivo: function(nombreVista) { 
        ...
        const btnInicio = document.getElementById('btn-menu-Inicio'); if (btnInicio) { btnInicio.classList.replace('btn-secundario', 'btn-primario'); }
        ...
        let categoriaPadre = nombreVista; for (const [padre, datos] of Object.entries(this.ModulosSistema)) { if (datos.items.some(i => i.vista === nombreVista)) { categoriaPadre = padre; break; } } 
    ...
    tienePermiso: function(rolesPermitidos) { if(!this.usuario) return false; if(this.usuario.rol === "Directivo" || this.usuario.rol === "Administrador") return true; if(rolesPermitidos.includes("ALL")) return true; return rolesPermitidos.includes(this.usuario.rol); },

    generarDashboardModulo: function(nombreCategoria) { 
        const modulo = this.ModulosSistema[nombreCategoria]; if(!modulo) return ""; let htmlTarjetas = ''; 
        modulo.items.forEach(item => { 
            ...
            htmlTarjetas += `<div class="col-md-6 col-xl-4 animate__animated animate__zoomIn"><div class="tarjeta-modulo p-4 h-100 shadow-sm bg-white ${claseBorde} ${tieneAcceso ? '' : 'opacity-75'}" ${onClickAttr}><div class="d-flex justify-content-between align-items-start mb-3"><div class="bg-${tieneAcceso ? 'primary' : 'secondary'} bg-opacity-10 p-3 rounded-4 d-inline-flex"><i class="bi ${item.icono} fs-2 text-${tieneAcceso ? 'primary' : 'secondary'}"></i></div>${!tieneAcceso ? '<i class="bi bi-lock-fill fs-4 text-danger"></i>' : '<i class="bi bi-arrow-right-short fs-4 text-muted opacity-50"></i>'}</div><h5 class="fw-bold text-dark mb-1">${item.vista}</h5><p class="small text-muted mb-0">${tieneAcceso ? 'Click para gestionar' : 'Requiere permisos: ' + item.roles[0]}</p></div></div>`; 
        }); 
        return `<div class="row mb-4 animate__animated animate__fadeInDown"><div class="col-12"><div class="banner-modulo p-4 p-md-5 text-white"><div class="burbuja-3d burbuja-1"></div><div class="burbuja-3d burbuja-2"></div><div class="burbuja-3d burbuja-3"></div><div class="row align-items-center position-relative z-1"><div class="col-md-8 text-center text-md-start mb-4 mb-md-0"><span class="badge bg-white text-primary mb-2 shadow-sm" style="font-weight: 800; letter-spacing: 1px;"><i class="bi ${modulo.icono} me-1"></i> CATEGORÍA</span><h2 class="fw-bolder mb-2 text-white" style="font-size: 2.2rem;">${nombreCategoria}</h2><p class="mb-0 text-white-50 fw-bold fs-5">${modulo.desc}</p></div></div></div></div></div><div class="row g-4">${htmlTarjetas}</div>`; 
    },
