...
const SUPABASE_URL = 'https://xztcoxdzzonsjvngozrp.supabase.co';
const SUPABASE_KEY = 'sb_publishable_YA4_3IRbc9Y3-lWAIwK4Cw_CU2R81eC';
...
window.ModulosSistema = {
    "Dirección y Sistema": { 
        ...
        items: [
            ...
            { vista: "Gestión de Registros", icono: "bi-database-fill-gear" }, // ✨ BOTÓN NUEVO
            { vista: "Panel de Control", icono: "bi-terminal-fill" } // ✨ MÓDULO NUEVO
        ] 
    },
    ...
    "Gestión Estudiantil": { 
        ...
        items: [
            ...
            { vista: "Gestión de Matrícula", icono: "bi-diagram-3-fill" } // ✨ NUEVO
        ] 
    },
    "Gestión Docente": { 
...
window.Aplicacion = {
    ...
    enviarRegistroInvitado: async function() { 
        ...
        try {
            ...
            // ✨ BARRERA DE MANTENIMIENTO: Tampoco deja entrar a los invitados
            const { data: sysConf } = await window.supabaseDB.from('conf_sistema').select('valor_booleano').eq('id_config', 'modo_mantenimiento').maybeSingle();
            if (sysConf && sysConf.valor_booleano === true) {
                this.ocultarCarga();
                return Swal.fire({
    ...
    // =========================================================
    // ✨ MOTOR GLOBAL DE NOTIFICACIONES PUSH (CADUCIDAD DIARIA Y TRES PITOS) ✨
    // =========================================================
